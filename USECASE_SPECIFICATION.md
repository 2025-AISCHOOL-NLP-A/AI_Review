...
**유스케이스 이름**: 로그인  
**유스케이스 ID**: UC-02  
**관련 요구사항**: FR-AUTH-004, FR-AUTH-005  
**우선순위**: 상  
**선행조건**: 회원가입 완료  
**관련 액터**: 사용자(User)
...
**유스케이스 이름**: 리뷰 데이터 수집  
**유스케이스 ID**: UC-03  
**관련 요구사항**: FR-RV-001(파일 업로드), FR-RV-002(컬럼 매핑), FR-RV-003(데이터 파싱/검증), FR-RV-004(리뷰 저장), FR-RV-005(이력 기록)  
**우선순위**: 상  
...
3. Python AI 서버에 분석 요청 (POST /api/v1/products/{product_id}/reviews/analysis)
4. AI 서버가 리뷰 데이터 조회
...
**유스케이스 이름**: 대시보드 조회  
**유스케이스 ID**: UC-09  
**관련 요구사항**: FR-DS-001, FR-DS-002, FR-DS-003  
**우선순위**: 상  
