...
# 🔹 워드클라우드 생성 함수
# ======================================
def generate_wordcloud_from_db(product_id: int, domain="game"):
    conn = get_connection()
    cursor = conn.cursor()

    # 1️⃣ 리뷰 불러오기
    cursor.execute("SELECT review_text FROM tb_review WHERE product_id = %s", (product_id,))
    ...
    okt = Okt()
    tokens = [
        t for t, pos in okt.pos(text_all)
    ...
    removed_ratio = round((before_count - after_count) / before_count * 100, 2) if before_count else 0

    print(f"🧹 불용어 제거 완료: {before_count - after_count}개 제거 ({removed_ratio}% 필터링됨)")
    ...
    # 6️⃣ 워드클라우드 생성 및 저장
    os.makedirs("backend/static/wordclouds", exist_ok=True)
    save_path = f"backend/static/wordclouds/product_{product_id}_wc.png"
...
if __name__ == "__main__":
    ...
    generate_wordcloud_from_db(product_id, domain="game")
